...
def split_text_by_punctuation(text):
    """Split text by periods and commas, keeping the punctuation"""
    import re
    # Split by . or , but keep them
    segments = re.split(r'([.,])', text)
    
    # Reconstruct segments with punctuation
    parts = []
    current = ""
...
def analyze_multi_segment_text(text, model, tokenizer):
    """Analyze text with multiple segments separated by punctuation"""
    segments = split_text_by_punctuation(text)
    
    # If only one segment, return single analysis
    if len(segments) <= 1:
        return predict_emotion_sentiment(text, model, tokenizer), None
    
    # Analyze each segment
    segment_results = []
    for segment in segments:
        if segment.strip():
            result = predict_emotion_sentiment(segment, model, tokenizer)
            if result:
    ...
    # Get overall emotion for full text
    overall_result = predict_emotion_sentiment(text, model, tokenizer)
    ...
    **Training Metrics:**
    ...
    if analyze_button and text_input.strip():
        with st.spinner("🔍 Analyzing..."):
            overall_result, segment_results = analyze_multi_segment_text(text_input, model, tokenizer)
        
        if overall_result:
            # Show segment analysis if multiple segments found
            if segment_results and len(segment_results) > 1:
                st.markdown("---")
                st.markdown("### 🔍 Segment-by-Segment Analysis")
                st.info("💡 Your text contains multiple segments separated by punctuation. Here's the emotion for each part:")
                
                for idx, seg in enumerate(segment_results, 1):
                    seg_color = get_emotion_color(seg['emotion'])
                    st.markdown(f"""
                    <div style='background-color: {seg_color}; color: white; padding: 1rem; border-radius: 10px; margin: 0.5rem 0;'>
                        <strong>Segment {idx}:</strong> "{seg['text']}"<br>
                        <span style='font-size: 1.2rem;'>{seg['emotion_emoji']} <strong>{seg['emotion'].upper()}</strong> ({seg['emotion_confidence']:.1%} confidence) | 
                        Sentiment: {seg['sentiment_emoji']} {seg['sentiment'].title()}</span>
                    </div>
                    """, unsafe_allow_html=True)
                
                st.markdown("---")
                st.markdown("### 📊 Overall Text Analysis")
                st.info("Below is the combined emotion and sentiment for your entire text:")
